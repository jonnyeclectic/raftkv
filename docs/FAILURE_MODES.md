...
real browser and measuring the cluster's answers rather than reading its code. The
provisioning row arrived the same way — pressed live on the kind demo, then traced from
the browser's error string back through three layers. Each row is now a regression test.
...
| `provision node` on the k8s demo answered `TypeError: Failed to fetch` — three defects stacked | The refusal designed for exactly this deployment never fired: a containerd pod leaves no `/.dockerenv` and sets no `container` var, so `_containerised()` waved the kind pod through. The spawn then died on the pod's read-only root filesystem, and the unmapped `OSError` became `{"error": "internal"}`. Worst, that 500 was *invisible*: the generic handler's response is sent from Starlette's `ServerErrorMiddleware`, **outside** the `CORSMiddleware` every routed response passes through, so a cross-origin tab (which every compose/k8s tab is — port-forwards on one origin, `127.0.0.1` targets on another) was not allowed to read it and reported only the browser's own `TypeError: Failed to fetch`. The same opacity hid every unhandled 500 from every endpoint, not just this one — the operator's only evidence was a string that pointed at the network, three layers from the cause | Four changes, one per layer plus posture. `_containerised()` also reads `KUBERNETES_SERVICE_HOST`, which the kubelet injects into every container unconditionally — so a pod now gets the 409 that names `kubectl scale statefulset/node`. Any `OSError` on the spawn path maps to the same readable 409, because a filesystem that refuses is a fact about the host, not a bug. The generic 500 now carries `access-control-allow-origin: *` (restating the app's own CORS policy), so nothing unhandled is ever invisible again. And `k8s/raftkv.yaml` no longer leaves the button dead: `RAFT_PROVISION_K8S=1` swaps the backend, so the same press PATCHes the StatefulSet's scale subresource through the API server — authenticated as the `raftkv-provisioner` ServiceAccount, bounded by an RBAC Role that allows get/patch on that one object's scale and nothing else, growing by exactly one under an optimistic lock (`src/raftkv/k8s_provision.py`). Process spawning stays refused in every container either way. `tests/test_provision.py::test_a_kubernetes_pod_counts_as_containerised`, `tests/test_provision.py::test_filesystem_errors_become_refusals_not_500s`, `tests/test_api.py::test_unhandled_exception_returns_structured_500`, `tests/test_deploy_topology.py::test_kubernetes_provisioning_uses_the_scale_backend_not_the_spawn_one`, `tests/test_k8s_provision.py::test_it_scales_by_exactly_one_under_the_read_lock`, and end to end in a real browser, `tests/test_e2e_provision_dashboard.py` |
| Eight pods Running, five visible — the k8s demo "stopped scaling" at node-5 | The scale backend was doing its job: every press added a replica and truthfully answered `node-N staged`. But the provision cap (12) lived in only one of the three places it governs. `make k8s-forward` hardcoded forwards for node-1..5, so `127.0.0.1:8006`–`8008` had nothing listening; and the dashboard's probe list stopped at 8005, so even a forwarded node-6 vanished from every freshly loaded page. Nothing errored anywhere — healthy pods, honest button, and a screen that said the feature was broken | `make k8s-forward` forwards every provisionable ordinal (a forward for a pod that does not exist yet retries quietly and comes alive when the button creates the pod), and the probe list climbs a ladder: an answering `800N` earns a knock on `800N+1`, capped at the provision ceiling, so a reloaded page walks back up to the top node with no query string while never probing more than one silent port. The cap's three copies — config default, forward range, ladder top — are pinned to each other. `tests/test_deploy_topology.py::test_every_provisionable_ordinal_is_forwardable_and_probeable`, `tests/test_dashboard_discover.py::test_an_answering_rung_earns_a_knock_on_the_next_port` |
