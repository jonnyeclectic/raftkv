...
| D2 | Omit snapshots (§7), membership changes (§6), PreVote, leader no-op on election win (thesis §6.4), ReadIndex — **name each in FAILURE_MODES.md with why-cut + production path** | Implementing them (weeks of work, more bug surface, violates "no bugs") | MIT 6.824 scopes labs the same way; Students' Guide flags snapshot/lastApplied subtleties as top bug farms | High |
