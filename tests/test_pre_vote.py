...
from raftkv.models import LogEntry, NoOp, RequestVoteRequest, RequestVoteResponse, Role
...
def test_a_node_that_knows_no_leader_grants_immediately(make_node):
    ...
    node = make_node()
    node.leader_id = None
    node._last_reset = time.monotonic()  # fresh, but protecting nobody
...
def test_a_lapsed_lease_grants_again(make_node):
    node = make_node()
    node.leader_id = "node-3"
    node._last_reset = time.monotonic() - 5.0  # the leader has gone quiet
