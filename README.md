...
| Performance regressions | `tests/test_log_repair.py`, `tests/test_gap_repair.py`, `tests/test_append_batching.py`, `tests/test_commit_scan.py` | Costs that were measured rather than guessed, each pinned so it cannot come back: one-index backtracking, the unbounded batch, and the O(N²) commit walk |
